...
for i, file in enumerate(files) :
    ...
    for stage in df_clean.newstages.unique() :
        for chan in df_clean.chan_name.unique() :
            ...
            diagHIlist.append(thisDemographics['diag.2'].iloc[0])
            diagHINIlist.append(thisDemographics['diag.3'].iloc[0])
...
df = pd.DataFrame(
    {
     "sub_id" : subidlist,
     "subtype" : subtypelist,
     "subtype_2" : diagHIlist,
     "subtype_3" : diagHINIlist,
     "genre" : genrelist,
     "age" : agelist,
     "ess" : esslist,
...
df.to_csv(f"{fig_dir}/df_allsw_exgausscrit_nobin_nt1_hi_cns.csv")
...
fig, ax = plt.subplots(
    nrows = 3, ncols = 1, sharex = True, sharey = True, figsize = (12, 16))
...
# LESS SLOW WAVES IN NT1 IN WAKE before sleep
# => Hypothesis : NO REST during wake => Daytime Sleepiness